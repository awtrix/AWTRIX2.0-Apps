...
Public Sub Initialize() As String
	...
	App.settings = CreateMap("Username":"","Password":"","Signature":"")
		
	'Setup Instructions. You can use HTML to format it
	App.setupDescription = $"
	 <b>1.</b> Login to your PayPal Business account<br>
	Go to “Activity” →"API Access" → "NVP/SOAP API Integration" → "Manage API Credentials"<br>
  		<b>2.</b> Copy API Username, API Password and Signature to the app’s settings<br>
  		<b>3.</b> Enjoy!<br>
	"$
	
	'define some tags to simplify the search in the Appstore
	App.tags = Array As String("Paypal", "balance", "money")
	
	'How many downloadhandlers should be generated
	App.downloads = 1
	
	'IconIDs from AWTRIXER. You can add multiple if you need more
	App.icons = Array As Int(204)
...
Sub App_startDownload(jobNr As Int)
	Select jobNr
		Case 1
			App.Download($"https://api-3t.paypal.com/nvp?USER=${App.get("Username")}&PWD=${App.get("Password")}&SIGNATURE=${App.get("Signature")}&VERSION=109.0&METHOD=GetBalance&RETURNALLCURRENCIES=1"$)
	End Select
End Sub
...
Sub App_evalJobResponse(Resp As JobResponse)
	Try
		If Resp.success Then
			Select Resp.jobNr
				Case 1
					Dim bal As String = Resp.ResponseString
					bal = bal.SubString2(bal.IndexOf("L_AMT0=")+7,bal.IndexOf("&L_CURRENCYCODE"))
					balance=bal.Replace("%2e",".")
			End Select
		End If
	Catch
...
Sub App_genFrame
	App.genSimpleFrame(balance,204,True,False,Null,True)
End Sub