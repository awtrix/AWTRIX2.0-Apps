...
Sub Class_Globals
	...
	Private AppName As String = "pr0gramm" 'change plugin name (must be unique)
	...
	Private appSettings As Map = CreateMap("Name":Null) 'needed Settings for this Plugin, parse Null if this setting should entered bny the user in the Apps Setup
...
Sub startDownload(nr As Int) As String
	...
	Select nr
		Case 1
			URL=("https://pr0gramm.com/api/profile/info?name="&Name&"&flags=1&self=false")
	End Select
	Return URL
End Sub
...
Sub genFrame As List
	commandList.Add(genText(benis))			'Fügt einen Befehl der Liste hinzu
